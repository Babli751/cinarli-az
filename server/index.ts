...
app.put("/api/categories/:id", authMiddleware, adminMiddleware, async (c) => {
  const { slug, name, description, parent_id, is_hidden, featured_product_id, banner_credit_months, banner_company_id } = await c.req.json();
  const icon = autoIcon(name);
  try {
    db.prepare("UPDATE categories SET slug=?, name=?, icon=?, description=?, parent_id=?, is_hidden=?, featured_product_id=?, banner_credit_months=?, banner_company_id=? WHERE id=?").run(slug, name, icon, description || "", parent_id || null, is_hidden ? 1 : 0, featured_product_id || null, banner_credit_months || null, banner_company_id || null, c.req.param("id"));
    return c.json({ ok: true });
  } catch (e: any) {
    if (e.message?.includes("UNIQUE")) return c.json({ error: "Bu slug artıq mövcuddur" }, 400);
    return c.json({ error: e.message }, 400);
